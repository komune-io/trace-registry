...
fun <E> SearchStream<E>.criterion(
    criterion: Criterion?, getField: (CriterionField<*>) -> MetamodelField<E, *>
): SearchStream<E> {
...
private fun <E> computeCriterion(
    criterion: Criterion?, getField: (CriterionField<*>
            ) -> MetamodelField<E, *>): Predicate<*>? {
    return when {
        criterion == null -> null
...
private fun <E, T> fieldCriterion(
    criterion: FieldCriterion<T>, getField: (CriterionField<*>) -> MetamodelField<E, *>
): Predicate<*>? {
    return match(getField(criterion.field) as MetamodelField<E, T>, criterion.match)
}

private fun <E> andCriterion(
    criterion: AndCriterion, getField: (CriterionField<*>) -> MetamodelField<E, *>
): Predicate<*> {
    return criterion.criteria.mapNotNull { computeCriterion(it, getField) }
        .reduce { acc, current -> acc.and(current as Predicate<in Any?>) }
}

private fun <E> orCriterion(
    criterion: OrCriterion, getField: (CriterionField<*>) -> MetamodelField<E, *>
): Predicate<*> {
    return criterion.criteria.mapNotNull { computeCriterion(it, getField) }
