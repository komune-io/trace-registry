  ...
  useEffect(() => {
    ...
    const $onDelete = (isBackspace: boolean) => {
      ...
      if (
        ...
        const isAtStartOfLayoutItem = !previousSibling && selection.anchor.offset === 0

        if (layoutItemNode) {
          const firstChild = layoutItemNode.getFirstChild()
          const moveChildBefore = () => {
            if (firstChild) {
              if ($isLayoutItemNode(previousContainer)) {
                previousContainer.append(firstChild)
              } else {
                container?.insertBefore(firstChild)
              }
              if (layoutItemNode.getChildrenSize() === 0) layoutItemNode.append($createParagraphNode())
            }
          }

          const islayoutItemEmpty = layoutItemNode.getChildrenSize() === 1 && $isParagraphNode(firstChild) && firstChild.isEmpty()

          if (islayoutItemEmpty) {
            if (!previousContainer && isBackspace) {
          ...
          if (isAtStartOfLayoutItem && isBackspace) {
            moveChildBefore()
          }
    ...
    return mergeRegister(
      ...
      // Delete layout on escape or delete command pressed on empty child
      editor.registerCommand(
        KEY_DELETE_COMMAND,
        () => $onDelete(false),
        COMMAND_PRIORITY_LOW,
...
const getLayoutItemInParentsAndFindPreviousSibling = (anchor: LexicalNode) => {
  let previousSibling: LexicalNode | null = null
  let layoutItemNode: LayoutItemNode | null = $isLayoutItemNode(anchor) ? anchor : null
  let ancestor: LexicalNode | null = anchor
  while (!layoutItemNode && ancestor) {
    const sibling = ancestor.getPreviousSibling()
    if (sibling && !previousSibling) previousSibling = sibling
    ancestor = ancestor.getParent()
    if ($isLayoutItemNode(ancestor)) {
      layoutItemNode = ancestor
    }
  }
  return {
    layoutItemNode,
